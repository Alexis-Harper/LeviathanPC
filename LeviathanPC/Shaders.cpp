...
GPU_ShaderBlock loadShaderProgram (const char *vert, const char *frag, 
								   Uint32 *v, Uint32 *f, Uint32 *p) 
{
	//Load vertex shader
	*v = GPU_LoadShader (GPU_VERTEX_SHADER, vert);
	...
	if (!*v) 
	{
		std::cout << "[-] GPU: " << GPU_GetErrorString (
	...
	*p = GPU_LinkShaders (*v, *f);
	...
	//Build block
	GPU_ShaderBlock block = GPU_LoadShaderBlock (*p, "gpu_Vertex",
												 "gpu_TexCoord",
												 "gpu_Color",
...
ShaderProgram loadShaderProgram (const char *vertexFile, 
								 const char *fragmentFile)
{
	struct ShaderProgram ret;

	ret.block = loadShaderProgram (vertexFile, fragmentFile, &ret.vertex,
								   &ret.fragment, &ret.program);
