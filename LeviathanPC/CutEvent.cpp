...
void CutEvent::testForEvent (GameState & state, Arena *& arena, 
							 Cutscene *& scene, Player & player) 
{
	...
	while (eventPtr) 
	{
		...
		if (Rectangle::rectIsColliding (eventPtr->cutEvent->hitbox, 
			player.getHitbox ())) 
		{
			//Change game state
			...
			/*
			TODO: If cutscene doesn't exist, delete it (this may not be 
			necessary after I finish cutscene code)
			*/
			if (scene != NULL) 
			{
				delete scene;
